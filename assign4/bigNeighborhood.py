...
def findDist(check, checked, inDist):
    x = 0
    y = 1
    z = 2
    dist = math.sqrt((check[x] - checked[x])**2 + (check[y] - checked[y])**2 + (check[z] - checked[z]**2))
    return 1 if dist < inDist else 0

def matrix(coords, distance):
    inDists = [] # nxn matrix of values, 0 if outside 25, 1 if inside 25
    for x in coords:
        dists = []
        for y in coords:
            if x != y: 
                dists.append(findDist(x, y, distance))
            else:
                dists.append(0)
...
def calculate(coords):
    inDists = matrix(coords) # nxn matrix of values, 0 if outside 25, 1 if inside 25
